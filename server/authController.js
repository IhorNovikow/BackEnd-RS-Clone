...
class authController {
	async registration(req, res) {
		try {
			...
			if (candidate) {
				return res.status(400).json({ message: 'Извините, пользователь с таким именем уже существует' });
			}
			const hashPassword = bcrypt.hashSync(password, 7);
			/* const userRole = await Role.findOne({ value: "USER" }) */
			const user = new User({ username, password: hashPassword, roles: 'USER', score: 0 });
			await user.save();
